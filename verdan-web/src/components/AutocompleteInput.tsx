  ...
  return (
    <div ref={ref} className="relative">
      <input
        ...
        className={
          className ||
          'w-full px-3 py-2 bg-bg-input border border-border rounded-lg text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-border-focus'
        }
      />
        ...
        <ul className="absolute z-50 w-full mt-1 max-h-48 overflow-y-auto bg-bg-secondary border border-border rounded-lg shadow-xl">
          {filtered.map((o) => (
            ...
            >
              ...
              {o.sublabel && (
                <span className="ml-2 text-text-muted text-xs font-mono">{o.sublabel}</span>
              )}
            </li>
          ))}
        </ul>
