...
import {
  ...
  DoorOpen, Calendar, LayoutDashboard, LogOut, Menu, X, BarChart3, Building2, MessageSquare, FileSearch, BookOpenCheck
} from 'lucide-react';
...
const getNavItems = (institutionLevel?: string): NavItem[] => [
  ...
  { to: '/student-portal', label: 'Elevportalen', icon: <BookOpenCheck size={20} />, roles: ['INSTITUTION_ADMIN', 'TEACHER', 'STUDENT'] },
];
...
export function Sidebar() {
  ...
  const filteredItems = getNavItems(user.institutionLevel).filter(item => item.roles.includes(user.role));
  ...
  return (
    ...
    <div className="flex h-screen">
      {/* Sidebar */}
      <aside className={`bg-sidebar-bg border-r border-border flex flex-col transition-all duration-300 ${collapsed ? 'w-16' : 'w-60'}`}>
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-border">
          ...
          <button onClick={() => setCollapsed(!collapsed)} className="p-1 rounded hover:bg-sidebar-hover text-text-secondary transition-colors">
            ...
            <div className="flex items-center gap-2 mb-2 px-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-col gap-0.5">
                  <span className={`inline-block text-[10px] font-semibold px-1.5 py-0.5 rounded ${roleBadgeColor} text-white w-fit`}>
                    {user.role}
                  </span>
