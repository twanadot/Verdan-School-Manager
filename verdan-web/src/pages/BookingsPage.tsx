    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-4 mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            value={selectedRoom}
            onChange={(e) =>
              setSelectedRoom(e.target.value === 'all' ? 'all' : parseInt(e.target.value))
            }
        ...
        <div className="flex items-center gap-1 ml-auto">
          ...
          <button
            onClick={today}
            className="px-3 py-1.5 text-xs font-medium rounded-md border border-border hover:bg-bg-hover text-text-secondary transition-colors"
          >
            I dag
          </button>
          <button
          ...
          <table className="w-full text-sm border border-border rounded-xl overflow-hidden">
            <thead>
              <tr className="bg-bg-card">
                <th className="px-3 py-2.5 text-left font-medium text-text-secondary border-b border-r border-border w-32">
                  Rom
                </th>
                {DAYS.map((day, i) => {
                  ...
                  return (
                    <th
                      key={day}
                      className={`px-3 py-2.5 text-center font-medium text-sm border-b border-r border-border last:border-r-0 ${isToday ? 'bg-accent/20 text-accent' : 'text-text-secondary'}`}
                    >
                      <div>{day}</div>
            ...
            <tbody>
              {rooms.map((room) => (
                <tr
                  key={room.id}
                  className="border-b border-border/50 last:border-b-0 hover:bg-bg-hover/30 transition-colors"
                >
                  ...
                  {DAYS.map((_, i) => {
                    ...
                    return (
                      <td
                        key={i}
                        className={`px-1.5 py-1.5 border-r border-border last:border-r-0 align-top ${isToday ? 'bg-accent/5' : hasBooking ? 'bg-bg-secondary' : 'bg-bg-secondary'}`}
                      >
                        {hasBooking ? (
                          <div className="space-y-1">
                            {bks.map((b) => {
                              ...
                              return (
                                ...
                                >
                                  <p
                                    className={`text-[10px] font-mono font-bold truncate ${isCancelled ? 'text-red-400 line-through' : 'text-accent'}`}
                                  >
                                    {b.subject}
                                  </p>
                                  <p
                                    className={`text-[9px] ${isCancelled ? 'text-red-400/60 line-through' : 'text-text-muted'}`}
                                  >
                                    {formatTime(b.startDateTime)}–{formatTime(b.endDateTime)}
                                  </p>
                                  {isCancelled && (
                                    <p className="text-[8px] text-red-400 font-semibold">
                                      Avspasering
                                    </p>
                                  )}
                                  {!isCancelled && b.programName && (
                                    <p className="text-[8px] text-purple-400 font-medium truncate">
                                      {b.programName}
                                    </p>
                                  )}
                                  {canCreate && (
                                    <div className="absolute top-0.5 right-0.5 flex gap-0.5 opacity-0 group-hover:opacity-100 transition-all">
                                      <button
                                        ...
                                        title={
                                          isCancelled ? 'Fjern avspasering' : 'Sett avspasering'
                                        }
        ...
        <div className="grid grid-cols-5 gap-px bg-border rounded-xl overflow-hidden border border-border">
          ...
          {DAYS.map((day, i) => {
            ...
            return (
              <div
                key={day}
                className={`px-3 py-2.5 text-center font-medium text-sm ${isToday ? 'bg-accent/20 text-accent' : 'bg-bg-card text-text-secondary'}`}
              >
                <div>{day}</div>
          ...
          {DAYS.map((_, i) => {
            ...
            return (
              <div
                key={i}
                className={`min-h-[160px] p-2 ${isToday ? 'bg-accent/5' : 'bg-bg-secondary'}`}
              >
                {bks.length === 0 ? (
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start justify-between gap-1">
                            <div>
                              <p
                                className={`text-xs font-mono font-semibold ${isCancelled ? 'text-red-400 line-through' : 'text-accent'}`}
                              >
                                {b.subject}
                              </p>
                              <div className="flex items-center gap-2 mt-0.5">
                                <p
                                  className={`text-[11px] ${isCancelled ? 'text-red-400/60 line-through' : 'text-text-secondary'}`}
                                >
                                  {formatTime(b.startDateTime)} – {formatTime(b.endDateTime)}
                                </p>
                                {!isCancelled && (
                                  <span className="text-[9px] text-accent/70 font-bold uppercase tracking-tighter opacity-70">
      ...
      {editTarget && (
        <EditBookingModal
          ...
          onConfirm={(data, series) =>
            updateMutation.mutate({ id: editTarget.id, booking: data, series })
          }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-bg-secondary border border-border rounded-xl p-6 w-full max-w-sm shadow-2xl">
        ...
        <p className="text-sm text-text-secondary mb-6">
          Vil du slette bare denne bookingen, eller hele serien (alle med fag{' '}
          <span className="font-mono text-accent">{booking.subject}</span> på samme tid)?
        </p>
        <div className="flex flex-col gap-3">
          <button
            onClick={() => onConfirm(true)}
            disabled={loading}
            className="w-full py-2.5 px-4 bg-danger hover:bg-danger/90 text-white rounded-lg text-sm font-medium transition-colors disabled:opacity-50"
          >
            Slett hele serien
          </button>
          <button
            ...
            disabled={loading}
            className="w-full py-2.5 px-4 border border-danger/50 text-danger hover:bg-danger/10 rounded-lg text-sm font-medium transition-colors disabled:opacity-50"
          >
          ...
          <button
            onClick={onClose}
            disabled={loading}
            className="w-full py-2.5 px-4 mt-2 text-text-secondary hover:bg-bg-hover rounded-lg text-sm font-medium transition-colors disabled:opacity-50"
          >
            Avbryt
          </button>
        </div>
...
function EditBookingModal({
  ...
  booking: Booking;
  onClose: () => void;
  onConfirm: (data: any, series: boolean) => void;
  loading: boolean;
}) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-bg-secondary border border-border rounded-xl p-6 w-full max-w-sm shadow-2xl">
        <button
          onClick={onClose}
          disabled={loading}
          className="absolute top-3 right-3 text-text-muted hover:text-text-primary disabled:opacity-50"
        >
          <X size={18} />
        </button>
        ...
        <div className="mb-4">
          ...
          <p className="text-sm font-mono p-2 bg-bg-input rounded border border-border opacity-70">
            {booking.subject} (Låst)
          </p>
        </div>
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div>
            <label className="block text-xs font-medium text-text-secondary mb-1">Dato</label>
            <input
              type="date"
          ...
          <div className="opacity-0">
            <label className="block text-xs font-medium text-text-secondary mb-1">_</label>
            <input
              type="date"
              disabled
              className="w-full px-3 py-2 bg-bg-input border border-border rounded-lg text-sm disabled:opacity-50"
            />
          </div>
        </div>
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div>
            <label className="block text-xs font-medium text-text-secondary mb-1">Starttid</label>
            <input
              type="time"
          ...
          <div>
            <label className="block text-xs font-medium text-text-secondary mb-1">Sluttid</label>
            <input
              type="time"
              value={form.endTime}
        ...
        <div className="flex flex-col gap-3">
          <button
            ...
            disabled={loading}
            className="w-full py-2.5 px-4 bg-accent hover:bg-accent-hover text-white rounded-lg text-sm font-medium transition-colors disabled:opacity-50"
          >
            Bruk på hele serien
          </button>
          <button
            ...
            disabled={loading}
            className="w-full py-2.5 px-4 border border-accent/50 text-accent hover:bg-accent/10 rounded-lg text-sm font-medium transition-colors disabled:opacity-50"
          >
...
function BookingFormModal({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const curDate = new Date(form.fromDate);
      ...
      setError(
        err.response?.data?.errors?.join('. ') ||
          ...
          'Kunne ikke opprette bookingserie',
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-bg-secondary border border-border rounded-xl p-6 w-full max-w-md shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-text-secondary mb-1.5">Rom</label>
            <select
              value={form.roomId}
              onChange={(e) => setForm({ ...form, roomId: parseInt(e.target.value) })}
              required
              disabled={loading}
              className="w-full px-3 py-2 bg-bg-input border border-border rounded-lg text-sm text-text-primary focus:outline-none focus:border-border-focus disabled:opacity-50"
            >
              ...
              {rooms.map((r) => (
                <option key={r.id} value={r.id}>
                  {r.roomNumber} ({r.roomType}, {r.capacity} plasser)
                </option>
              ))}
            </select>
          </div>
          ...
          <div>
            ...
            <input
              ...
              disabled={loading}
              placeholder="F.eks. Eksamen, Gruppearbeid"
              className="w-full px-3 py-2 bg-bg-input border border-border rounded-lg text-sm text-text-primary focus:outline-none focus:border-border-focus disabled:opacity-50"
            />
          </div>
