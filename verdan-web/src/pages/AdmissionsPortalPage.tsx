...
import {
  getPortalListings, getMyApplications, submitApplications, withdrawApplication,
  getAdmissionPeriods, createAdmissionPeriod, closeAdmissionPeriod, deleteAdmissionPeriod,
  reopenAdmissionPeriod, bulkPublishPrograms,
  processAdmissions, getAdmissionOverview, getAdmissionRequirements, setAdmissionRequirement,
  enrollAccepted
} from '../api/admissions';
import { getMyGraduation, getPrograms } from '../api/programs';
import type { Program } from '../types';
import type {
  PortalListing, ApplicationResponse, AdmissionPeriod,
  ProgramApplicantSummary, AdmissionRequirement, ProcessingResult
} from '../api/admissions';
import { PageHeader } from '../components/PageHeader';
import { LoadingState, EmptyState } from '../components/LoadingState';
import { ConfirmDialog } from '../components/ConfirmDialog';
import {
  Search, X, Send, XCircle, CheckCircle2, Clock, AlertTriangle, GraduationCap, Building2,
  Plus, ChevronDown, ChevronRight, Settings, Play, Lock, Users, Filter, Trash2, RotateCcw, Upload
} from 'lucide-react';
...
export function AdmissionsPortalPage() {
  ...
  if (isStudent && !gradLoading && !isGraduated) {
    return (
      <div>
        <PageHeader title="Søknadsportal" description="Felles opptaksplattform for alle institusjoner" />
        <div className="flex flex-col items-center justify-center py-20">
          <div className="w-16 h-16 rounded-2xl bg-accent/10 flex items-center justify-center mb-4">
            <Lock size={32} className="text-accent" />
          </div>
          <h2 className="text-xl font-semibold text-text-primary mb-2">Søknadsportalen er ikke tilgjengelig ennå</h2>
          <p className="text-text-muted text-center max-w-md">
            Søknadsportalen åpnes når du er uteksaminert fra din nåværende institusjon.
            Dine karakterer vil automatisk brukes når du søker.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div>
      ...
      {tab === 'portal' && <PortalBrowse search={search} setSearch={setSearch} levelFilter={levelFilter} setLevelFilter={setLevelFilter} />}
  ...
  return (
    <button onClick={onClick}
      className={`flex items-center gap-1.5 px-4 py-2 text-sm font-medium rounded-md transition-colors ${
        active ? 'bg-accent text-white shadow-sm' : 'text-text-secondary hover:text-text-primary hover:bg-bg-hover'
      }`}>
      {children}
    </button>
  );
...
// ══════════════════════════════════════════════════════════════════════════════
function PortalBrowse({ search, setSearch, levelFilter, setLevelFilter }: {
  search: string; setSearch: (s: string) => void; levelFilter: string; setLevelFilter: (s: string) => void;
}) {
  const { user } = useAuth();
  const queryClient = useQueryClient();
  ...
  const { data: listings = [], isLoading } = useQuery({
    ...
    queryFn: () => getPortalListings(
      user?.role === 'STUDENT' ? user?.institutionLevel : undefined
    ),
  });
  ...
  const filtered = useMemo(() => {
    ...
    return listings.filter(l => {
      const matchSearch = !s || l.institutionName.toLowerCase().includes(s) ||
        l.programName.toLowerCase().includes(s) || l.periodName.toLowerCase().includes(s);
  ...
  const publicListings = useMemo(() => filtered.filter(l => l.ownership !== 'PRIVATE'), [filtered]);
  const privateListings = useMemo(() => filtered.filter(l => l.ownership === 'PRIVATE'), [filtered]);
  ...
  const renderProgramCard = (p: PortalListing) => (
    <div key={`${p.periodId}-${p.programId}`}
      ...
      <div className="flex-1">
        ...
        {p.prerequisites && (
          <div className="flex flex-wrap gap-1 mt-1.5">
            {p.prerequisites.split(',').map(t => t.trim()).filter(Boolean).map((tag, i) => (
              <span key={i} className="px-2 py-0.5 bg-orange-400/10 text-orange-400 text-[11px] font-medium rounded-full">
                {tag}
              </span>
            ))}
          </div>
      ...
      {user?.role === 'STUDENT' && (
        <button onClick={() => setApplyTarget(p)}
  ...
  const renderInstitutionGroup = (instName: string, programs: PortalListing[], ownership: 'PUBLIC' | 'PRIVATE') => (
    <div key={instName} className="bg-bg-card border border-border rounded-xl overflow-hidden">
      <div className="px-5 py-4 border-b border-border/50 flex items-center gap-3">
        <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
          ...
          <Building2 size={20} className={ownership === 'PRIVATE' ? 'text-amber-400' : 'text-accent'} />
        </div>
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <h3 className="text-base font-semibold text-text-primary">{instName}</h3>
            <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${
              ownership === 'PRIVATE' ? 'bg-amber-400/10 text-amber-400' : 'bg-blue-400/10 text-blue-400'
            }`}>
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          <select value={ownershipFilter} onChange={e => setOwnershipFilter(e.target.value as '' | 'PUBLIC' | 'PRIVATE')}
            className="px-3 py-2.5 bg-bg-input border border-border rounded-lg text-sm text-text-primary">
          ...
          {publicGrouped.size > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <div>
                  <h2 className="text-base font-semibold text-text-primary">Offentlige institusjoner</h2>
                  <p className="text-xs text-text-muted">Felles opptakspool — du kan velge opptil 5 programmer på tvers av alle offentlige institusjoner</p>
                </div>
              </div>
          ...
          {privateGrouped.size > 0 && (
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <div>
                  <h2 className="text-base font-semibold text-text-primary">Private institusjoner</h2>
                  <p className="text-xs text-text-muted">Uavhengig opptak — du kan søke fritt på hver privat institusjon</p>
                </div>
              </div>
              <div className="space-y-4">
      ...
      {/* Quick apply modal */}
      {applyTarget && (
        <QuickApplyModal listing={applyTarget} onClose={() => setApplyTarget(null)}
          onApplied={() => { setApplyTarget(null); queryClient.invalidateQueries({ queryKey: ['portalListings'] }); }} />
      )}
    </div>
  );
}
...
// ── Quick Apply Modal ──────────────────────────────────────────────────────────
function QuickApplyModal({ listing, onClose, onApplied }: {
  listing: PortalListing; onClose: () => void; onApplied: () => void;
}) {
  const queryClient = useQueryClient();
  const [choices, setChoices] = useState<{ programId: number; programName: string; institutionName: string }[]>([
    { programId: listing.programId, programName: listing.programName, institutionName: listing.institutionName }
  ]);
  ...
  const addChoice = (l: PortalListing) => {
    if (choices.length >= maxChoices) { toast.error(`Maks ${maxChoices} valg`); return; }
    setChoices([...choices, { programId: l.programId, programName: l.programName, institutionName: l.institutionName }]);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      await submitApplications(listing.periodId,
      ...
      <div className="relative bg-bg-secondary border border-border rounded-xl w-full max-w-lg shadow-2xl flex flex-col max-h-[85vh]">
        ...
        <div className="overflow-y-auto flex-1 p-5 space-y-5">
          ...
          <div className="p-3 bg-accent/5 border border-accent/20 rounded-lg text-xs text-text-secondary">
            ...
            {listing.fromLevel === 'UNIVERSITET' && 'Dine universitets-/høyskolekarakterer brukes.'}
              ...
              {choices.map((c, i) => (
                <div key={c.programId} className="flex items-center gap-2 px-3 py-2 bg-accent/5 border border-accent/20 rounded-lg">
                  ...
                  <div className="flex-1 min-w-0">
                    <span className="text-sm font-medium text-text-primary block truncate">{c.programName}</span>
                    {isPublic && <span className="text-[11px] text-text-muted">{c.institutionName}</span>}
                  </div>
          ...
          {availablePrograms.length > 0 && (
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-1.5">
                {isPublic ? 'Legg til fra alle offentlige institusjoner' : 'Legg til flere programmer'}
              </label>
              <div className="border border-border rounded-lg max-h-48 overflow-y-auto">
                {availablePrograms.map(l => (
                  <button key={l.programId} onClick={() => addChoice(l)}
                    ...
                    <div className="min-w-0">
                      <span className="text-sm font-medium text-text-primary block truncate">{l.programName}</span>
                      {isPublic && <span className="text-[11px] text-text-muted">{l.institutionName}</span>}
                    </div>
        ...
        <div className="border-t border-border p-5 flex justify-end gap-3">
          ...
          <button onClick={handleSubmit} disabled={choices.length === 0 || submitting}
...
function MyApplications() {
  ...
  const withdrawMut = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['myApplications'] }); toast.success('Søknad trukket'); setWithdrawTarget(null); },
  ...
  return (
    <div className="space-y-6">
      {[...grouped.entries()].map(([periodId, apps]) => {
        ...
        return (
          <div key={periodId} className="bg-bg-card border border-border rounded-xl overflow-hidden">
                ...
                return (
                  <div key={app.id} className="flex items-center justify-between px-5 py-3 hover:bg-bg-hover/30 transition-colors">
                    ...
                    <div className="flex items-center gap-3">
                      <span className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium ${sc.color}`}>
                        {sc.icon} {sc.label}
                      </span>
...
// ══════════════════════════════════════════════════════════════════════════════
function AdminPanel({ showCreate, setShowCreate }: { showCreate: boolean; setShowCreate: (v: boolean) => void }) {
  const queryClient = useQueryClient();
...
function PeriodCard({ period, expanded, onToggle }: {
  ...
  const [enrollmentResult, setEnrollmentResult] = useState<{ enrolled: number; skipped: number; total: number } | null>(null);
  ...
  const closeMut = useMutation({
    ...
    onSuccess: () => { invalidateAll(); toast.success('Opptaksperiode stengt'); },
    onError: () => toast.error('Kunne ikke stenge periode'),
  });

  const reopenMut = useMutation({
    mutationFn: () => reopenAdmissionPeriod(period.id),
    onSuccess: () => { invalidateAll(); toast.success('Opptaksperiode gjenåpnet'); },
    onError: () => toast.error('Kunne ikke gjenåpne'),
  });

  const deleteMut = useMutation({
    mutationFn: () => deleteAdmissionPeriod(period.id),
    onSuccess: () => { invalidateAll(); toast.success('Opptaksperiode slettet'); setConfirmDelete(false); },
    ...
    <div className="bg-bg-card border border-border rounded-xl overflow-hidden">
      ...
      {expanded && (
        <div className="border-t border-border px-5 py-4 space-y-4">
          <div className="flex gap-2 flex-wrap">
            ...
            {period.status === 'PROCESSED' && (
              <button onClick={() => enrollMut.mutate()} disabled={enrollMut.isPending}
                ...
                <CheckCircle2 size={13} /> {enrollMut.isPending ? 'Melder inn...' : 'Meld inn aksepterte elever'}
              </button>
            )}
          ...
          {enrollmentResult && (
            <div className="p-4 bg-green-400/5 border border-green-400/20 rounded-lg">
              ...
              <div className="flex gap-6">
                ...
                <div className="text-center"><p className="text-xl font-bold text-orange-400">{enrollmentResult.skipped}</p><p className="text-xs text-text-muted">Hoppet over</p></div>
                <div className="text-center"><p className="text-xl font-bold text-text-primary">{enrollmentResult.total}</p><p className="text-xs text-text-muted">Totalt akseptert</p></div>
              </div>
              <p className="text-xs text-text-muted mt-2">Elever er overført til ny institusjon og innmeldt i programmer.</p>
            </div>
          )}
          ...
          <ConfirmDialog open={confirmDelete} title="Slett opptaksperiode"
            message={`Er du sikker på at du vil slette "${period.name}"? Alle søknader og krav slettes permanent.`}
            onConfirm={() => deleteMut.mutate()} onCancel={() => setConfirmDelete(false)}
...
function ApplicantOverview({ periodId }: { periodId: number }) {
  ...
  if (!overview || overview.length === 0) return <p className="text-sm text-text-muted italic">Ingen programmer publisert ennå. Trykk "Publiser programmer" for å legge ut opptak.</p>;
          ...
          <div className="px-4 py-3 bg-bg-hover/30 flex items-center justify-between">
            <div>
              ...
              <span className="ml-2 text-xs text-text-muted">
                {prog.totalApplicants} søkere{prog.maxStudents != null ? ` / ${prog.maxStudents} plasser` : ''}
...
function RequirementsModal({ periodId, onClose }: { periodId: number; onClose: () => void }) {
  ...
  const handleBulkPublish = async () => {
    ...
    setBulkLoading(true);
    try {
      const count = await bulkPublishPrograms(periodId, availablePrograms.map(p => p.id));
      invalidateAll();
      ...
      <div className="relative bg-bg-secondary border border-border rounded-xl w-full max-w-lg shadow-2xl flex flex-col max-h-[85vh]">
        <div className="flex items-center justify-between p-5 border-b border-border">
          <div>
            ...
            <p className="text-sm text-text-muted mt-0.5">Velg programmer å legge ut på søknadsportalen</p>
          </div>
          <button onClick={onClose} className="text-text-muted hover:text-text-primary"><X size={18} /></button>
        </div>
        ...
        <div className="overflow-y-auto flex-1 p-5 space-y-4">
          ...
          {requirements.length > 0 && (
            <div>
              <h3 className="text-sm font-semibold text-text-secondary mb-2">Publiserte programmer</h3>
              <div className="space-y-1.5">
                {requirements.map(r => (
                  <div key={r.id} className="flex items-center justify-between px-3 py-2.5 border border-green-400/20 bg-green-400/5 rounded-lg">
                    <div className="flex items-center gap-2">
          ...
          {availablePrograms.length > 0 && (
            <div className="border-t border-border pt-4">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-semibold text-text-secondary">Publiser alle på én gang</h3>
                <button onClick={handleBulkPublish} disabled={bulkLoading}
                  className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-accent hover:bg-accent-hover text-white rounded-lg transition-colors disabled:opacity-50">
                  <Upload size={13} /> {bulkLoading ? 'Publiserer...' : `Publiser alle (${availablePrograms.length})`}
                </button>
              </div>
                  ...
                  <div key={p.id} className="px-3 py-2 border border-border/50 rounded-lg text-sm">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-xs text-text-muted space-x-2">
                        ...
                        {p.minGpa == null && p.maxStudents == null && !p.prerequisites && <span className="italic">Ingen krav satt</span>}
                      </div>
                    </div>
                    {p.prerequisites && (
                      <div className="flex flex-wrap gap-1 mt-1">
                        {p.prerequisites.split(',').map(t => t.trim()).filter(Boolean).map((tag, i) => (
                          <span key={i} className="px-1.5 py-0.5 bg-orange-400/10 text-orange-400 text-[10px] font-medium rounded-full">{tag}</span>
                        ))}
                      </div>
                    )}
                  </div>
          ...
          {availablePrograms.length > 0 && (
            <div className="border-t border-border pt-4 space-y-3">
              <h3 className="text-sm font-semibold text-text-secondary">Eller legg til enkeltvis</h3>
              ...
              <div className="flex gap-3">
                <div className="flex-1">
                  ...
                  <input type="number" step="0.1" min="1" max="6" value={minGpa} onChange={e => setMinGpa(e.target.value)}
                    placeholder="Valgfritt" className="w-full px-3 py-2 bg-bg-input border border-border rounded-lg text-sm text-text-primary" />
                </div>
                <div className="flex-1">
          ...
          {availablePrograms.length === 0 && requirements.length > 0 && (
            <p className="text-sm text-text-muted italic border-t border-border pt-4">Alle programmer er allerede publisert ✓</p>
          )}
        </div>
      </div>
    </div>
