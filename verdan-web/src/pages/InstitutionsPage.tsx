...
export function InstitutionsPage() {
  ...
  return (
    <div>
      ...
      {filtered.length === 0 ? (
        <EmptyState
          title={search ? 'Ingen treff' : 'Ingen institusjoner funnet'}
          message={
            search ? 'Prøv et annet søkeord' : 'Legg til en ny institusjon for å komme i gang'
          }
        />
      ) : (
              ...
              <div key={level}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pl-2">
                  {items.map((inst) => (
                    <div
                      key={inst.id}
                      className="bg-bg-secondary p-4 rounded-xl border border-border group hover:border-accent/30 transition-colors"
                    >
...
function InstitutionFormModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.error || err.response?.data?.errors?.join(', ') || 'Kunne ikke lagre',
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-bg-secondary border border-border rounded-xl p-6 w-full max-w-sm shadow-2xl">
        ...
        <h2 className="text-lg font-semibold text-text-primary mb-4">
          {isEditing ? 'Rediger institusjon' : 'Ny institusjon'}
        </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => setForm((p) => ({ ...p, ownership: 'PUBLIC' }))}
              ...
              <button
                type="button"
                onClick={() => setForm((p) => ({ ...p, ownership: 'PRIVATE' }))}
                className={`flex-1 px-3 py-2 text-sm font-medium rounded-lg border transition-colors ${
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-4 py-2 text-sm font-medium bg-accent hover:bg-accent-hover text-white rounded-lg disabled:opacity-50 transition-colors"
            >
