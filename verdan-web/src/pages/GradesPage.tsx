...
function calculateAverage(grades: Grade[], level: SubjectLevel): string | null {
  ...
  if (isNumeric) {
    const valid = grades
      .map((g) => parseInt(formatGradeValue(g.value, level)))
      .filter((v) => !isNaN(v));
    ...
    const valid = grades
      .map((g) => map[formatGradeValue(g.value, level).toUpperCase()])
...
function numericAverage(
  ...
  for (const g of grades) {
    ...
    if (!isNaN(num) && num >= 1 && num <= 6) {
      sum += num;
      count++;
    } else if (letterToNum[display.toUpperCase()] !== undefined) {
      sum += letterToNum[display.toUpperCase()];
      count++;
    }
  }
...
class GradesErrorBoundary extends React.Component<
    ...
    if (this.state.error)
      return (
        <div className="p-8 text-red-400">
          <h2 className="text-lg font-bold mb-2">Feil i karaktersiden</h2>
          <pre className="text-sm bg-bg-card p-4 rounded-lg overflow-auto">
...
function GradesPageInner() {
  ...
  const subjectLevelMap = useMemo(() => {
    ...
    subjects.forEach((s) => {
      map[s.code.toUpperCase()] = s.level;
    });
    return map;
  }, [subjects]);
  ...
  const subjectYearMap = useMemo(() => {
    ...
    subjects.forEach((s) => {
      if (s.yearLevel) map[s.code.toUpperCase()] = s.yearLevel;
    });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('Karakter slettet');
  ...
  const getGradeYearLevel = (g: Grade) =>
    g.yearLevel || subjectYearMap[g.subject.toUpperCase()] || null;
  ...
  const allFiltered = grades.filter(
    (g) =>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {grpGrades.map((g) => {
                    ...
                    const gradeYL =
                      getGradeYearLevel(g) || subjectYearMap[g.subject.toUpperCase()] || '';
                    return (
                      <tr
                        key={g.id}
                      ...
                      >
                        {!hideStudentCol && (
                          <td className="px-4 py-3 text-text-primary">
                            {g.studentName || g.studentUsername}
                          </td>
                        )}
                        <td className="px-4 py-3 font-mono text-accent">{g.subject}</td>
                        <td className="px-4 py-3 text-text-secondary text-xs">
                          {gradeYL ? (
                            <span className="bg-bg-hover px-2 py-0.5 rounded-md">
                              {formatYearLabel(gradeYL)}
                            </span>
                          ) : (
                            '—'
                          )}
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`inline-block px-2.5 py-1 rounded-lg font-bold text-sm ${gradeColor(displayValue, level)}`}
                            title={
                              g.blockedByAbsence && g.originalValue
                                ? `Opprinnelig: ${g.originalValue} — Blokkert pga. fraværsgrense`
                                : undefined
                            }
                          >
                            {displayValue}
                          ...
                          {g.retake && (
                            <span className="ml-2 text-[10px] font-medium text-amber-400 bg-amber-400/10 px-1.5 py-0.5 rounded">
                              Privatist
                            </span>
                          )}
                        </td>
  ...
  return (
    <div>
      <PageHeader
        title={isStudent ? 'Mine Karakterer' : 'Karakterer'}
        description={
          isStudent
            ? 'Din utdanningshistorikk og karakteroversikt'
            : 'Karakterer sortert etter klasse'
        }
        action={
          canWrite ? (
            <button
      ...
      {viewTab === 'current' && (
        <>
          {isStudent ? (
            /* Student view */
            filtered.length === 0 ? (
              <EmptyState title="Ingen karakterer funnet" />
            ) : (
              <div className="space-y-8">
                {/* Vitnemål Summary Card */}
                <VitnemaalCard
                  grades={filtered}
                  subjectLevelMap={subjectLevelMap}
                        ...
                        <div className="flex items-center gap-3">
                          {avg && (
                            <div className="bg-bg-card px-3 py-1.5 rounded-lg border border-border flex items-center gap-2 shadow-sm">
                              ...
                              <span
                                className={`font-bold ${isNumericLevel(level) ? 'text-accent' : gradeColor(avg, level).split(' ')[0]}`}
                              >
                                {avg}
                              </span>
          ...
          programs.length > 0 ? (
            <div className="space-y-3">
              {[...programs]
                .sort((a, b) => {
                  const ma = a.name.match(/^(VG\d+|\d+)(.*)/),
                    mb = b.name.match(/^(VG\d+|\d+)(.*)/);
                  if (ma && mb) {
                    const na = ma[1].startsWith('VG')
                      ? parseInt(ma[1].slice(2)) + 100
                      : parseInt(ma[1]);
                    const nb = mb[1].startsWith('VG')
                      ? parseInt(mb[1].slice(2)) + 100
                      : parseInt(mb[1]);
                    const diff = na - nb;
                ...
                .map((p) => {
                  ...
                  const programGradeCount = students.reduce(
                    (sum, s) => sum + (gradesByStudent[s.username]?.length || 0),
                    0,
                  );
                    ...
                    >
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-xs text-text-muted bg-bg-hover rounded-full px-2.5 py-0.5">
                            {studentCount} elever
                          </span>
                          {programGradeCount > 0 && (
                            <span className="text-xs text-accent bg-accent/10 rounded-full px-2.5 py-0.5">
                              {programGradeCount} karakterer
                            </span>
                          )}
                        </div>
                      </button>
                      ...
                      {isExpanded && (
                        <div className="border-t border-border">
                          {students.length === 0 ? (
                            <p className="text-sm text-text-muted italic px-5 py-4">
                                  ...
                                  <div key={s.userId}>
                                    ...
                                    >
                                      <div className="flex items-center gap-3">
                                        ...
                                        {s.yearLevel && (
                                          <span className="text-xs text-accent bg-accent/10 rounded px-1.5 py-0.5">
                                            {s.yearLevel}. trinn
                                          </span>
                                        )}
                                      </div>
                                      <span className="text-xs text-text-muted">
                                        {studentGrades.length > 0
                                          ? `${studentGrades.length} karakter${studentGrades.length !== 1 ? 'er' : ''}`
                                          : 'Ingen karakterer'}
                                      </span>
                                    </button>
                                    ...
                                    {isStudentExpanded && studentGrades.length === 0 && (
                                      <p className="text-sm text-text-muted italic px-10 pb-3">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-bg-card border border-border rounded-2xl shadow-2xl p-6 w-full max-w-md mx-4">
            ...
            <p className="text-sm text-text-muted mb-4">
              Sett ny karakter for{' '}
              <span className="font-mono text-accent">{retakeTarget.subject}</span> —{' '}
              {retakeTarget.studentName}
              {retakeTarget.originalValue && (
                <span className="block mt-1 text-xs">
                  Opprinnelig karakter: <strong>{retakeTarget.originalValue}</strong>
                </span>
              )}
            </p>
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button
                onClick={() =>
                  retakeValue.trim() &&
                  retakeMutation.mutate({ id: retakeTarget.id, value: retakeValue.trim() })
                }
...
function EducationHistoryView({
  history,
  subjectYearMap,
}: {
  history: EducationLevel[];
  subjectYearMap: Record<string, string>;
}) {
  if (history.length === 0) {
                ...
                <div className="flex-1 bg-bg-card border border-border rounded-xl overflow-hidden">
                  ...
                  <div className="px-5 py-4 border-b border-border/50 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-text-muted mt-0.5">
                        {eduLevel.institutionName} · {eduLevel.grades.length} fag
                      </p>
                    </div>
                    <div className="flex items-center gap-3">
                  ...
                  <div className="divide-y divide-border/30">
                    {sortedTrinns.map((trinn) => {
                      ...
                      const trinnAllPassing =
                        eduLevel.level === 'UNGDOMSSKOLE' ||
                        trinnGrades.every((g) => gradeToNum(g.value) >= 2);
                        ...
                        <div key={trinn}>
                          ...
                          {trinnGrades.map((g) => {
                            ...
                            return (
                              <div
                                key={g.id}
                                className={`flex items-center justify-between px-5 py-2.5 ${isIV ? 'bg-red-500/5' : isFailing ? 'bg-red-400/5' : ''}`}
                              >
                                <div className="flex items-center gap-3">
                                  <span className="text-sm font-mono text-accent w-24">
                                    {g.subject}
                                  </span>
                                  {g.dateGiven && (
                                    <span className="text-xs text-text-muted">{g.dateGiven}</span>
                                  )}
                                </div>
                                <div className="flex items-center gap-2">
                                  {isIV ? (
                                    <span
                                      className="px-2.5 py-1 rounded-lg font-bold text-sm text-red-500 bg-red-500/15 border border-red-500/30"
                                      title={
                                        g.originalValue
                                          ? `Opprinnelig: ${g.originalValue}`
                                          : undefined
                                      }
                                    >
                                      IV
...
function GradeFormModal({
  ...
  const programsForYear = useMemo(() => {
    ...
    return allPrograms.filter((p) => {
      const members = allProgramMembers[p.id];
      const memberMatch = members?.students?.some(
        (s) => !s.graduated && s.yearLevel === form.yearLevel,
      );
    ...
    if (needsProgramSelect) {
      ...
      const programStudentUsernames = new Set(
        members.students
          .filter(
            (s) =>
              !s.graduated && (!form.yearLevel || !s.yearLevel || s.yearLevel === form.yearLevel),
          )
          .map((s) => s.username),
      );
  ...
  const gradeOptions = useNumeric ? ['6', '5', '4', '3', '2', '1'] : ['A', 'B', 'C', 'D', 'E', 'F'];

  // Year level options based on institution level
  const yearLevelOptions = useMemo(() => {
    if (instLevel === 'UNGDOMSSKOLE')
      return [
    ...
    if (instLevel === 'FAGSKOLE')
      return [
        { value: '1', label: '1. året' },
        { value: '2', label: '2. året' },
      ];
    return [
      { value: 'BACHELOR_1', label: 'Bachelor 1' },
      { value: 'BACHELOR_2', label: 'Bachelor 2' },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing) {
        await updateGrade(grade!.id, form);
        toast.success('Karakter oppdatert');
      } else {
        await createGrade(form);
        toast.success('Karakter registrert');
      }
      onSaved();
    } catch (err: any) {
      setError(
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-bg-secondary border border-border rounded-xl p-6 w-full max-w-md shadow-2xl">
        ...
        <h2 className="text-lg font-semibold mb-4">
          {isEditing ? 'Rediger Karakter' : 'Sett Karakter'}
        </h2>
        {error && (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {needsProgramSelect && form.yearLevel && (
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-1.5">
                ...
                {programsForYear.length === 0 && (
                  <span className="text-text-muted font-normal">
                    {' '}
                    (ingen linjer på dette trinnet)
                  </span>
                )}
              </label>
              <select
          ...
          <div>
            <label className="block text-sm font-medium text-text-secondary mb-1.5">
              Elev
              {needsProgramSelect && !selectedProgramId && form.yearLevel && (
                <span className="text-text-muted font-normal"> (velg linje først)</span>
              )}
              {!needsProgramSelect && !form.yearLevel && (
                <span className="text-text-muted font-normal"> (velg trinn først)</span>
              )}
              {((needsProgramSelect && selectedProgramId) ||
                (!needsProgramSelect && form.yearLevel)) &&
                filteredStudentOptions.length === 0 && (
                  <span className="text-text-muted font-normal"> (ingen elever)</span>
                )}
            </label>
            <AutocompleteInput
          ...
          <div>
            <label className="block text-sm font-medium text-text-secondary mb-1.5">
              Fag{' '}
              {form.studentUsername && subjectOptions.length === 0 && (
                <span className="text-text-muted font-normal">(ingen fag tildelt)</span>
              )}
            </label>
            <AutocompleteInput
              value={form.subject}
            ...
            <label className="block text-sm font-medium text-text-secondary mb-1.5">
              ...
              {selectedSubject ? (
                <span className={`font-normal ${useNumeric ? 'text-accent' : 'text-purple-400'}`}>
                  ({useNumeric ? '1-6' : 'A-F'} —{' '}
                  {selectedSubject.level.charAt(0) + selectedSubject.level.slice(1).toLowerCase()})
                </span>
              ) : (
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            >
              {loading ? 'Lagrer...' : isEditing ? 'Oppdater' : 'Registrer'}
            </button>
          </div>
        </form>
      </div>
    ...
    <div className="bg-gradient-to-br from-accent/5 via-bg-card to-bg-card border border-accent/20 rounded-xl p-6">
      ...
      <div className="mt-5 flex gap-6 flex-wrap">
        ...
        <div className="text-center">
          <p
            className={`text-3xl font-bold ${overallGpa ? gpaColor(overallGpa) : 'text-text-muted'}`}
          >
