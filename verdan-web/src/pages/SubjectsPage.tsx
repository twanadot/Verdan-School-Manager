...
const _gradeScale = (level: SubjectLevel) =>
...
export function SubjectsPage() {
  ...
  const deleteProgramMut = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success(`${config.programLabel} slettet`);
  ...
  const unlinkMut = useMutation({
    mutationFn: ({ programId, subjectId }: { programId: number; subjectId: number }) =>
      removeSubjectFromProgram(programId, subjectId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['programs'] });
      toast.success('Fag fjernet fra program');
    },
  ...
  const filteredPrograms = programs
    ...
    .sort((a, b) => {
      const ma = a.name.match(/^(\d+)(.*)/),
        mb = b.name.match(/^(\d+)(.*)/);
      if (ma && mb) {
        const diff = parseInt(ma[1]) - parseInt(mb[1]);
    ...
    <div>
          ...
          isAdmin ? (
            <div className="flex gap-2">
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-bg-secondary hover:bg-bg-hover border border-border text-text-primary text-sm font-medium rounded-lg transition-colors"
              >
      ...
      <div className="relative max-w-sm mb-6">
        ...
        <input
          ...
          placeholder="Søk etter fag eller program..."
                ...
                <div className="flex items-center justify-between px-5 py-4 hover:bg-bg-hover/50 transition-colors">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {program.programType === 'YRKESFAG' && (
                          <span className="shrink-0 text-[10px] font-semibold px-2 py-0.5 rounded-full bg-orange-400/10 text-orange-400">
                            Yrkesfag
                          </span>
                        )}
                      </div>
                  ...
                  {isAdmin && (
                    <div className="flex items-center gap-1 ml-3">
                      <button
                        onClick={() => setMembersProgram(program)}
                        title="Administrer medlemmer"
                        className="p-1.5 rounded-md hover:bg-bg-hover text-text-muted hover:text-info transition-colors"
                      >
                        <Users size={14} />
                      </button>
                      <button
                        onClick={() => setLinkingProgram(program)}
                        title={`Legg til fag i ${config.programLabel.toLowerCase()}`}
                        ...
                        <tbody>
                          {sortSubjects(program.subjects, config).map((s) => {
                            ...
                            return (
                              ...
                              >
                                <td className="px-5 pl-12 py-2.5 font-mono font-medium text-accent text-xs">
                                  {s.code}
                                </td>
                                ...
                                <td className="px-4 py-2.5 text-text-secondary text-xs">
                                  {s.yearLevel ? yearLevelLabel(s.yearLevel, config) : '—'}
                                </td>
                                <td className="px-4 py-2.5">
                                  <div className="flex gap-1">
                                    {fullSubject && (
                                      <button
                                        onClick={() => setMembersSubject(fullSubject)}
                                        title="Medlemmer"
                                        className="p-1.5 rounded-md hover:bg-bg-hover text-text-muted hover:text-info transition-colors"
                                      >
                                        <Users size={14} />
                                    ...
                                    {isAdmin && (
                                      <>
                                        ...
                                        <button
                                          ...
                                          title="Fjern fra dette programmet"
            ...
            <div className="bg-bg-card border border-border rounded-xl overflow-hidden">
              <div className="flex items-center gap-3 px-5 py-4 border-b border-border/50">
                ...
                <div>
                  ...
                  <p className="text-xs text-text-muted">
                    {unlinkedSubjects.length} fag som ikke er tilknyttet noen{' '}
                    {config.programLabel.toLowerCase()}
                  </p>
                </div>
              </div>
              <table className="w-full text-sm">
                ...
                <tbody>
                  {unlinkedSubjects.map((s) => (
                    <tr
                      ...
                      className="border-b border-border/20 hover:bg-bg-hover/30 transition-colors"
                    >
                      <td className="px-5 py-2.5 font-mono font-medium text-accent text-xs">
                        {s.code}
                      </td>
      ...
      {membersSubject && (
        <SubjectMembersModal
          subject={membersSubject}
          isAdmin={isAdmin}
          onClose={() => setMembersSubject(null)}
        />
      )}
      ...
      <ConfirmDialog
        ...
        title={`Slett ${config.programLabel.toLowerCase()}`}
        ...
        onConfirm={() => deleteProgramTarget && deleteProgramMut.mutate(deleteProgramTarget.id)}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => deleteSubjectTarget && deleteSubjectMut.mutate(deleteSubjectTarget.id)}
        onCancel={() => setDeleteSubjectTarget(null)}
        loading={deleteSubjectMut.isPending}
      />

      {/* Promotion Modal */}
      {showPromotion && <PromotionModal config={config} onClose={() => setShowPromotion(false)} />}
    </div>
  );
}
...
function PromotionModal({ config, onClose }: { config: LevelConfig; onClose: () => void }) {
  ...
  const [undoResult, setUndoResult] = useState<{ promoted: number; graduated: number } | null>(
    null,
  );
  ...
  const handleUndo = async () => {
    ...
    try {
      ...
      toast.success(
        `Oppflytting angret: ${res.promoted} flyttet tilbake, ${res.graduated} gjenåpnet`,
      );
    } catch {
                ...
                <div>
                  ...
                  <div className="rounded-lg border border-border overflow-hidden">
                    {preview!.promotions.map((a, i) => (
                      <div
                        key={`p-${a.userId}-${a.programId}`}
                        className={`flex items-center justify-between px-3 py-2.5 ${i < preview!.promotions.length - 1 ? 'border-b border-border/50' : ''}`}
                      >
                        <div>
                        ...
                        <div className="flex items-center gap-1.5 text-xs">
                          <span className="px-1.5 py-0.5 rounded bg-bg-hover text-text-secondary font-medium">
                            {yearLevelLabel(a.fromYear, config)}
                          </span>
                          <span className="text-text-muted">→</span>
                          <span className="px-1.5 py-0.5 rounded bg-accent/10 text-accent font-medium">
                            {a.toYear ? yearLevelLabel(a.toYear, config) : '—'}
                          </span>
                        </div>
                      </div>
                    ))}
                ...
                <div>
                  ...
                  <div className="rounded-lg border border-green-500/30 overflow-hidden">
                    {preview!.graduations.map((a, i) => (
                      <div
                        key={`g-${a.userId}-${a.programId}`}
                        className={`flex items-center justify-between px-3 py-2.5 bg-green-500/5 ${i < preview!.graduations.length - 1 ? 'border-b border-green-500/20' : ''}`}
                      >
                        <div>
                          <span className="text-sm font-medium text-text-primary">
                            {a.fullName}
                        ...
                        <div className="flex items-center gap-2">
                          <span className="px-1.5 py-0.5 rounded bg-bg-hover text-text-secondary text-xs font-medium">
                            {yearLevelLabel(a.fromYear, config)}
                          </span>
    ...
    <div className="bg-bg-card border border-border rounded-xl overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          {expanded ? (
            <ChevronDown size={18} className="text-danger shrink-0" />
          ) : (
            <ChevronRight size={18} className="text-text-muted shrink-0" />
          )}
          ...
          <div>
            <h3 className="text-sm font-semibold text-text-primary">
              Alle fag ({subjects.length})
            </h3>
            <p className="text-xs text-text-muted">
              Administrer og slett fag permanent fra alle {config.programLabelPlural.toLowerCase()}
            </p>
          </div>
        </div>
      </button>
                    ...
                    >
                      ...
                      <td className="px-4 py-2.5">
                        <div className="flex gap-1">
                          ...
                          <button
                            onClick={() => onDelete(s)}
                            title="Slett fag permanent fra alle programmer"
                            className="p-1.5 rounded-md hover:bg-danger/10 text-text-muted hover:text-danger transition-colors"
                          >
                            <Trash2 size={14} />
                          </button>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEditing) {
        ...
        toast.success(`${config.programLabel} oppdatert`);
      } else {
        await createProgram(form);
        toast.success(`${config.programLabel} opprettet`);
      }
      onSaved();
    } catch (err: any) {
      setError(
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-bg-secondary border border-border rounded-xl p-6 w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <h2 className="text-lg font-semibold text-text-primary mb-4">
          {isEditing
            ? `Rediger ${config.programLabel.toLowerCase()}`
            : `Opprett ${config.programLabel.toLowerCase()}`}
        </h2>
        {error && (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {isVGS && (
            <div>
              ...
              <p className="text-xs text-text-muted mt-1.5">
                {form.programType === 'STUDIEFORBEREDENDE' &&
                  'VG1 → VG2 → VG3 (generell studiekompetanse)'}
                {form.programType === 'YRKESFAG' &&
                  'VG1 → VG2 → uteksaminert (fagbrev / kompetansebevis)'}
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full px-3 py-2 bg-bg-input border border-border rounded-lg text-sm text-text-primary focus:outline-none focus:border-border-focus resize-none"
            />
          </div>
          ...
          {showAdvancedFields && (
            <div className="border-t border-border pt-4">
              <p className="text-sm font-medium text-text-secondary mb-3">
                Opptakskrav (for søknadsportalen)
              </p>
                ...
                {tags.length > 0 && (
                  <div className="flex flex-wrap gap-1.5">
                    {tags.map((tag, i) => (
                      <span
                        key={i}
                        className="flex items-center gap-1 px-2.5 py-1 bg-accent/10 text-accent text-xs font-medium rounded-full"
                      >
                        {tag}
                        <button
                          type="button"
          ...
          {showAdvancedFields && (
            <div className="pt-3 border-t border-border/50">
              ...
              <label className="flex items-center gap-2 mb-3">
                <input
                  type="checkbox"
                  checked={form.attendanceRequired || false}
                  disabled={isVGS}
                  onChange={(e) =>
                    setForm({
                      ...form,
                      attendanceRequired: e.target.checked,
                      minAttendancePct: e.target.checked ? form.minAttendancePct || 80 : null,
                    })
                  }
                ...
                <span className="text-sm text-text-secondary">
                  Fraværsgrense aktiv{' '}
                  {isVGS && (
                    <span className="text-xs text-text-muted">(VGS: alltid 10% grense)</span>
                  )}
                </span>
              </label>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm({
                        ...form,
                        minAttendancePct: e.target.value ? parseInt(e.target.value) : null,
                      })
                    }
                  ...
                  <p className="text-xs text-text-muted mt-1">
                    {form.minAttendancePct
                      ? `Maks ${100 - form.minAttendancePct}% ugyldig fravær tillatt`
                      : 'Sett grensen'}
                  </p>
                </div>
              )}
...
function ProgramMembersModal({
  ...
  const [transferTarget, setTransferTarget] = useState<{ userId: number; name: string } | null>(
    null,
  );
  ...
  const invalidate = () =>
    queryClient.invalidateQueries({ queryKey: ['programMembers', program.id] });

  const removeMut = useMutation({
    mutationFn: (userId: number) => removeProgramMember(program.id, userId),
    onSuccess: () => {
      toast.success('Medlem fjernet (fag-koblinger oppdatert)');
      invalidate();
    },
  ...
  const handleBulkAdd = async () => {
    ...
    for (const userId of selectedUserIds) {
      try {
        await addProgramMember(program.id, {
          userId,
          role: addRole,
          yearLevel: addRole === 'STUDENT' ? addYearLevel || undefined : undefined,
        });
        added++;
      } catch {
        failed++;
  ...
  const renderMemberList = (title: string, icon: React.ReactNode, memberList: ProgramMember[]) => (
    <div>
      ...
      {memberList.length === 0 ? (
        <p className="text-text-muted text-sm italic pl-1">
          Ingen {title.toLowerCase()} tilknyttet
        </p>
      ) : (
        <div className="rounded-lg border border-border overflow-hidden">
          {memberList.map((m, i) => (
            <div
              key={m.id}
              className={`flex items-center justify-between px-3 py-2.5 ${i < memberList.length - 1 ? 'border-b border-border/50' : ''} hover:bg-bg-hover/40 transition-colors`}
            >
              ...
              {isAdmin && (
                <div className="flex gap-1 shrink-0">
                  ...
                  <button
                    ...
                    className="p-1 rounded hover:bg-danger/10 text-text-muted hover:text-danger transition-colors disabled:opacity-50 shrink-0"
                    title="Fjern"
                  >
                    <X size={13} />
                  </button>
        ...
        <div className="flex items-center justify-between p-5 border-b border-border">
          <div>
            ...
            <p className="text-sm text-text-muted mt-0.5">
              ...
              {program.subjects.length > 0 && <span> · {program.subjects.length} fag</span>}
            </p>
          </div>
          <button
            onClick={onClose}
              ...
              <div className="space-y-3">
                <div className="flex gap-2">
                  <select
                    value={addRole}
                    onChange={(e) => {
                      setAddRole(e.target.value as 'STUDENT' | 'TEACHER');
                      setSelectedUserIds(new Set());
                      setSearchUser('');
                    }}
                ...
                {availableUsers.length > 0 ? (
                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="text-xs text-text-muted">
                        {availableUsers.length} tilgjengelige · {selectedUserIds.size} valgt
                      </span>
                      <div className="flex gap-2">
                        <button
                          type="button"
                    ...
                    <div className="rounded-lg border border-border overflow-hidden max-h-48 overflow-y-auto">
                      {availableUsers.map((u, i) => (
                        <label
                          key={u.id}
                          className={`flex items-center gap-3 px-3 py-2.5 cursor-pointer hover:bg-bg-hover/50 transition-colors ${i < availableUsers.length - 1 ? 'border-b border-border/30' : ''}`}
                        >
                          <input
                            type="checkbox"
                  ...
                  <p className="text-xs text-text-muted italic">
                    Ingen tilgjengelige {addRole === 'STUDENT' ? 'elever' : 'lærere'} funnet
                  </p>
                )}
                ...
                <div className="flex gap-2 justify-end">
                  <button
                    ...
                    className="px-3 py-1.5 text-sm border border-border text-text-secondary rounded-lg hover:bg-bg-hover transition-colors"
                  >
                    Avbryt
                  </button>
                  <button
                  ...
                  >
                    {bulkLoading
                      ? 'Legger til...'
                      : `Legg til ${selectedUserIds.size > 0 ? `(${selectedUserIds.size})` : ''}`}
                  </button>
                </div>
              </div>
            )}
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center">
          ...
          <div className="relative bg-bg-secondary border border-border rounded-xl p-6 w-full max-w-sm shadow-2xl">
            ...
            <p className="text-sm text-text-muted mb-4">
              Flytt <span className="text-text-primary font-medium">{transferTarget.name}</span> fra{' '}
              <span className="text-accent font-medium">{program.name}</span> til:
            </p>
            <select
              value={transferProgramId}
              onChange={(e) => setTransferProgramId(e.target.value ? Number(e.target.value) : '')}
              ...
              <button
                onClick={() =>
                  ...
                  transferMut.mutate({
                    ...
                    toProgramId: transferProgramId as number,
                  })
                }
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-4">
          {available.length === 0 ? (
            <p className="text-center text-text-muted text-sm py-6">
              Ingen tilgjengelige fag funnet
            </p>
          ) : (
            <div className="space-y-1">
              {available.map((s) => (
                <div
                  key={s.id}
                  className="flex items-center justify-between px-3 py-2.5 rounded-lg hover:bg-bg-hover transition-colors"
                >
                  <div>
...
function SubjectMembersModal({
  ...
  const invalidate = () =>
    queryClient.invalidateQueries({ queryKey: ['subjectMembers', subject.code] });

  const assignMutation = useMutation({
    mutationFn: () => assignMember(subject.code, { username: addUsername, role: addRole }),
    onSuccess: () => {
      toast.success(`${addUsername} lagt til som ${addRole.toLowerCase()}`);
      invalidate();
      setAdding(false);
      setAddUsername('');
      setAddRole('STUDENT');
        ...
        <div className="overflow-y-auto flex-1 p-5 space-y-5">
          {isLoading ? (
            ...
            <>
              <MemberSection
                title="Lærere"
                icon={<BookOpen size={14} className="text-warning" />}
                members={members?.teachers ?? []}
                isAdmin={isAdmin}
              ...
              <MemberSection
                title="Elever"
                icon={<GraduationCap size={14} className="text-info" />}
              ...
              <div className="space-y-3">
                  ...
                  <div className="flex-1">
                    ...
                    {addUsername && (
                      <p className="text-xs text-accent mt-1">
                        Valgt: <span className="font-mono">{addUsername}</span>
                      </p>
                    )}
                  </div>
                  <select
                ...
                <div className="flex gap-2 justify-end">
                  ...
                  <button
                    ...
                    disabled={!addUsername || assignMutation.isPending}
                    className="px-3 py-1.5 text-sm bg-accent hover:bg-accent-hover text-white rounded-lg transition-colors disabled:opacity-50"
                  >
      ...
      {members.length === 0 ? (
        ...
        <div className="rounded-lg border border-border overflow-hidden">
          {members.map((u, i) => (
            <div
              key={u.username}
              className={`flex items-center justify-between px-3 py-2.5 ${i < members.length - 1 ? 'border-b border-border/50' : ''} hover:bg-bg-hover/40 transition-colors`}
            >
              <div>
                <span className="text-sm font-medium text-text-primary">
                  {u.firstName} {u.lastName}
              ...
              {isAdmin && (
                <button
                  ...
                  disabled={removing}
                  className="p-1 rounded hover:bg-danger/10 text-text-muted hover:text-danger transition-colors disabled:opacity-50"
                  title="Fjern"
                >
                  <X size={13} />
                </button>
...
function SubjectFormModal({
  ...
  const [form, setForm] = useState<SubjectRequest>({
    ...
    level: subject?.level || 'UNIVERSITET',
    teacherUsername: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isEditing) {
      if (selectedProgramIds.size === 0) {
        setError(`Du må velge minst én ${config.programLabel.toLowerCase()}`);
        setLoading(false);
        return;
      }
      if (!form.teacherUsername || form.teacherUsername.trim() === '') {
        setError('Du må tildele en lærer');
        setLoading(false);
        return;
      }
    }
        ...
        toast.success(
          'Fag opprettet' +
            (selectedProgramIds.size > 0
              ? ` og lagt til i ${selectedProgramIds.size} ${config.programLabel.toLowerCase()}${selectedProgramIds.size > 1 ? 'er' : ''}`
              : ''),
        );
      }
      onSaved();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-bg-secondary border border-border rounded-xl p-6 w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <h2 className="text-lg font-semibold text-text-primary mb-4">
          {isEditing ? 'Rediger fag' : 'Opprett fag'}
        </h2>
        {error && (
          <div className="mb-4 p-3 bg-danger/10 border border-danger/20 rounded-lg text-danger text-sm">
            {error}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-text-secondary mb-1.5">Kode</label>
            <input
              value={form.code}
          ...
          {!isEditing && programs.length > 0 && (
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-1.5">
                Legg til i {config.programLabel.toLowerCase()}{' '}
              ...
              {selectedProgramIds.size > 0 && (
                <p className="text-xs text-accent mt-1.5">
                  {selectedProgramIds.size} {config.programLabel.toLowerCase()}
                  {selectedProgramIds.size > 1 ? 'er' : ''} valgt
                </p>
              )}
            </div>
          )}
          ...
          {!isEditing && (
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-1.5">
                Tildel lærer <span className="text-danger">*</span>
              </label>
              <AutocompleteInput
